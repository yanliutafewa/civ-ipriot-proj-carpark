...
class Sensor:
    ...
    def __init__(self, displayer, car):
        ...
        l_plate_value = tk.Label(displayer, text=f"{car.car_plate}", font=('Arial', 16))
        l_model = tk.Label(displayer, text="car_model :", font=('Arial', 16))
        l_model_value = tk.Label(displayer, text=f"{car.car_model}", font=('Arial', 16))
        ...
        l_entry_time_value = tk.Label(displayer, text=f"{car.entry_time}", font=('Arial', 16))
        l_entry_car_park = tk.Label(displayer, text="entry_car_park :", font=('Arial', 16))
        l_entry_car_park_name = tk.Label(displayer, text=f"{car.entry_car_park_name}", font=('Arial', 16))
        ...
        def entry_car_park():
            ...
            msg = f"{selected_number.get()},in,{current_time},{car.car_plate}"
            self.publisher.publish_msg(msg)
            print(f"publish: topic=CarAct, msg={msg}")
            car.entry_car_park_id = selected_number.get()
            ...
            for line in self.tv_car_parks.get_children():
                if self.tv_car_parks.item(line)['values'][0] == int(selected_number.get()):
                    ...
                    car.entry_car_park_name = self.tv_car_parks.item(line)['values'][1]
                    # set entry_time and car_park_name on screen
                    l_entry_time_value.config(text=current_time)
                    l_entry_car_park_name.config(text=car.entry_car_park_name)
                    messagebox.showinfo("Message", f"Welcome to [{car.entry_car_park_name}].")
        ...
        # 'exit' button
        def exit_car_park():
            if car.entry_car_park_id == self.DISPLAY_INIT:
                messagebox.showinfo("Error", "You haven't got into any car park.")
                return
            current_time = datetime.datetime.now().strftime('%d-%m-%Y %H:%M:%S')
            msg = f"{car.entry_car_park_id},out,{current_time},{car.car_plate}"
            self.publisher.publish_msg(msg)
            print(f"publish: topic=CarAct, msg={msg}")
            l_entry_time_value.config(text=self.DISPLAY_INIT)
            l_entry_car_park_name.config(text=self.DISPLAY_INIT)
            messagebox.showinfo("Message", f"Thank you for using [{car.entry_car_park_name}].")
            car.entry_car_park_id = self.DISPLAY_INIT
            car.entry_car_park_name = self.DISPLAY_INIT
